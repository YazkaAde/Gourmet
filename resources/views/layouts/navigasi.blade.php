    ...
    <div class="overflow-y-auto py-5 px-3 h-full bg-white border-r border-gray-200">
        ...
        <ul class="space-y-2">
            @if($role === 'admin')
                ...
                {{-- Customer Management --}}
                <li>
                    <a href="{{ route('admin.blacklist.index') }}" class="flex items-center p-2 text-base font-normal text-gray-900 rounded-lg hover:bg-gray-100 group {{ request()->routeIs('admin.blacklist*') ? 'bg-gray-100' : '' }}">
                        <svg class="w-6 h-6 text-gray-500 transition duration-75 group-hover:text-gray-900" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
